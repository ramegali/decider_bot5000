...
# need to clean up functions... too recursive
# add more options for Yelp terms?
...
def display_results(results):
    ...
    for categories in results:
        print "%s \n\tRATING: %s \n\tPRICE: %s \n\tADDRESS: %s, %s %s %s \n\tPHONE: %s \n" %(results[i]['name'], results[i]['rating'], results[i]['price'], 
            results[i]['location']['address1'], results[i]['location']['city'], results[i]['location']['state'], results[i]['location']['zip_code'],
...
def your_destiny(results):
    ...
    print "Alright, here's what I picked for you I hope you're sastisfied: %s\n" %(results[destiny]['name'])
